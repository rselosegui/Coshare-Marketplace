...
const s1 = `{/* Captured Media Preview */}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                <button 
                  onClick={handleCapture}
                  className="flex-1 py-4 bg-gray-50 dark:bg-gray-900 text-primary dark:text-gray-50 rounded-2xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 border border-gray-100 dark:border-gray-800"
                >
                  <Video className="w-4 h-4" />
                  Take Video
                </button>
                <button className="flex-1 py-4 bg-gray-50 dark:bg-gray-900 text-primary dark:text-gray-50 rounded-2xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 border border-gray-100 dark:border-gray-800">
...
// Need to also handle the 'PenTool' import if it exists, otherwise use 'Edit3'
if (content.includes('PenTool')) {
    // already imported
} else if (content.includes('import {') && content.includes('lucide-react')) {
     content = content.replace('import {', 'import { PenTool, ');
}

fs.writeFileSync('src/App.tsx', content);
console.log('Fixed Handover Protocol Step 4');
