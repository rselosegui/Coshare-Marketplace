...
const replacements = [
  ...
  { match: /\bbg-primary\b(?! dark:bg-| \/|text-)/g, replace: 'bg-primary dark:bg-gray-900' },
];
...
content = content.replace(/dark:bg-[^\s'"]+ dark:bg-[^\s'"]+/g, (match) => match.split(' ')[0]);
content = content.replace(/dark:text-[^\s'"]+ dark:text-[^\s'"]+/g, (match) => match.split(' ')[0]);
